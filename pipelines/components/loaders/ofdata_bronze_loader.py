...
class OfdataBronzeLoader(Loader):
    """Загружает данные в коллекцию MongoDB (ofdata_bronze.raw_data)."""
    ...
    def load(self, data: Generator[Dict[str, Any], None, None]) -> None:
        logging.info(f"💾 Starting data load to MongoDB collection: {OFDATA_BRONZE_COLLECTION}")
        batch = []
        batch_size = 500
    ...
    def _process_batch(self, mongo: MongoConnector, batch: list) -> None:
        try:
            mongo.insert(batch, upsert_key=["inn"], ordered=False)
