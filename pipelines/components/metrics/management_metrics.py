...
from settings import MANAGEMENT_TABLE


class ManagementMetrics(Metrics):
    """Сбор и сохранение метрик выполнения пайплайна Management Gold"""
    ...
    def log_post_run_metrics(self, transformed_data=None):
        ...
        # Подсчет записей
        with self._pg.with_defaults(schema="public", table=MANAGEMENT_TABLE):
            result = self._pg.execute(f'SELECT COUNT(*) FROM "{MANAGEMENT_TABLE}"')
            total_records = result[0][0] if result else 0

            result = self._pg.execute(
                f'SELECT COUNT(*) FROM "{MANAGEMENT_TABLE}" WHERE created_at >= %s',
                (self._start_time.isoformat(),)
            )
            new_records = result[0][0] if result else 0

            result = self._pg.execute(
                f'SELECT COUNT(*) FROM "{MANAGEMENT_TABLE}" WHERE updated_at >= %s AND updated_at != created_at',
                (self._start_time.isoformat(),)
            )
            updated_records = result[0][0] if result else 0

        metrics_record = {
            "pipeline_name": "management_gold",
