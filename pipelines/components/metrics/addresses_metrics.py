...
from settings import ADDRESSES_TABLE


class AddressesMetrics(Metrics):
    """Сбор и сохранение метрик выполнения пайплайна Addresses Gold"""
    ...
    def log_post_run_metrics(self, transformed_data=None):
        ...
        # Подсчет записей
        with self._pg.with_defaults(schema="public", table=ADDRESSES_TABLE):
            result = self._pg.execute(f'SELECT COUNT(*) FROM "{ADDRESSES_TABLE}"')
            total_records = result[0][0] if result else 0

            result = self._pg.execute(
                f'SELECT COUNT(*) FROM "{ADDRESSES_TABLE}" WHERE created_at >= %s',
                (self._start_time.isoformat(),)
            )
            new_records = result[0][0] if result else 0

            result = self._pg.execute(
                f'SELECT COUNT(*) FROM "{ADDRESSES_TABLE}" WHERE updated_at >= %s AND updated_at != created_at',
                (self._start_time.isoformat(),)
            )
            updated_records = result[0][0] if result else 0

        metrics_record = {
            "pipeline_name": "addresses_gold",
