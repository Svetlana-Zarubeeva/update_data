...
class OfdataGoldMetrics(Metrics):
    """Сбор и сохранение метрик выполнения пайплайна Legal Entities Gold"""
    ...
    def log_post_run_metrics(self, transformed_data=None):
        ...
        with self._pg.with_defaults(schema="public", table=OFDATA_GOLD_TABLE):
            result = self._pg.execute(f'SELECT COUNT(*) FROM "{OFDATA_GOLD_TABLE}"')
            total_records = result[0][0] if result else 0

            result = self._pg.execute(
                f'SELECT COUNT(*) FROM "{OFDATA_GOLD_TABLE}" WHERE created_at >= %s',
                (self._start_time.isoformat(),)
            )
            new_records = result[0][0] if result else 0

            result = self._pg.execute(
                f'SELECT COUNT(*) FROM "{OFDATA_GOLD_TABLE}" WHERE updated_at >= %s AND updated_at != created_at',
                (self._start_time.isoformat(),)
            )
